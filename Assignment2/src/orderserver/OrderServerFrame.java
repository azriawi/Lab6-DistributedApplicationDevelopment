...
public class OrderServerFrame extends JFrame {

	private static final long serialVersionUID = 1L;
	
	// Private components
	private JLabel lblServerStatus;
	private JTextArea textFieldArea;
	...
	public OrderServerFrame () {
		...
		// Row, Column
		this.textFieldArea  = new JTextArea(20, 70);
		textFieldArea.setForeground(Color.GREEN);
		textFieldArea.setBackground(Color.BLACK);
	...
	private JPanel getRequestStatusPanel () {
		...
		textFieldArea.setFont(new Font("Courier", Font.PLAIN, 15));
	...
	public void updateRequestStatus (String status) {
		...
		String currentText = this.textFieldArea.getText();
		textFieldArea.setEditable(true);
		
		// Display the latest status on top
		status += "\n > " + currentText;
		this.textFieldArea.setText(status);
		textFieldArea.setEditable(false);
	}
